...
export const afterDefault = ({
  ...
  &::after {
    ...
    background: ${backgroundColor && THEME_COLORS_DEFAULT[backgroundColor]};
    clip-path: polygon(0% 0%, 100% 0%, 100% 50%, 0% 100%, 0% 0%);
    transform: translateX(${translate}0);
    transform: translateX(${translate}0);
    transform: ${transform && transform}
      (${transitionValue && transitionValue} ${unit && unit});
    transition: ${transitionProperty && transitionProperty}
      ${transitionDuration && transitionDuration}
      ${secondsDuration || secondsDuration} ${transitionTimingFunction}
      ${transitionDelay} ${secondsDelay || secondsDelay};
